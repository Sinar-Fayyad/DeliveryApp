        ...
        ? rows.map(a => {
            ...
            return `<li>
              <div class="order-meta">
                <span>Order <code style="font-size:0.78rem; color:var(--text-muted);">#${a.orderId.slice(0, 8)}</code></span>
                <span class="order-status"><span class="badge badge-${statusClass}">${a.status}</span></span>
              </div>
              <div class="order-actions">
                <form method="POST" action="/courrier/status" class="inline-form">
                  <input type="hidden" name="assignmentId" value="${a.assignmentId}" />
                  <select name="status">
                    <option value="${OrderStatus.PREPARING}"${a.status === OrderStatus.PREPARING ? " selected" : ""}>${OrderStatus.PREPARING}</option>
                    <option value="${OrderStatus.ONTHEWAY}"${a.status === OrderStatus.ONTHEWAY ? " selected" : ""}>${OrderStatus.ONTHEWAY}</option>
                    <option value="${OrderStatus.DELIVERED}"${a.status === OrderStatus.DELIVERED ? " selected" : ""}>${OrderStatus.DELIVERED}</option>
                  </select>
